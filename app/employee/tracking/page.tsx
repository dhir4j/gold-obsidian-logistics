...
export default function EmployeeTrackingPage() {
  ...
  const handleSearchById = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/shipments/${id}`);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    handleSearchById(trackingId.trim());
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-serif text-3xl font-bold text-[#F5F5F0] mb-2">Track Shipment</h1>
        <p className="text-[#F5F5F0]/60">Enter your tracking ID to view real-time shipment status</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="font-serif text-2xl font-semibold text-[#F5F5F0] mb-2">Shipment Details</h2>
                <p className="text-[#F5F5F0]/60">Tracking ID: {trackingData.shipment_id_str}</p>
              </div>
              <span className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium border ${getStatusColor(trackingData.status)}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#F5F5F0]/50">{trackingData.receiver_address_city}</p>
                </div>
              </div>
              <div className="space-y-4">
                <div>
          ...
          {/* Timeline */}
          <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 p-8">
            <h2 className="font-serif text-2xl font-semibold text-[#F5F5F0] mb-6">Tracking History</h2>
      ...
      {!trackingData && !error && (
        <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 p-12 text-center">
          ...
          <h3 className="font-serif text-xl font-semibold text-[#F5F5F0] mb-2">Track Your Shipment</h3>
          <p className="text-[#F5F5F0]/60">Enter a tracking ID above to view shipment details and tracking history</p>
        </div>
      )}
    </div>
  );
