...
export default function EmployeeDocumentsPage() {
  ...
  const fetchShipments = async (email: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const validShipments = data.filter((s: Shipment) => s.status !== "Pending Payment");
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-3xl font-bold text-[#F5F5F0] mb-2">Documents & Invoices</h1>
        <p className="text-[#F5F5F0]/60">View and download invoices, AWB labels, and shipping documents</p>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 overflow-hidden">
        <div className="border-b border-[#C5A059]/20 p-6">
          <h2 className="font-serif text-2xl font-semibold text-[#F5F5F0]">Available Documents</h2>
          <p className="text-sm text-[#F5F5F0]/60 mt-1">Download shipping labels and invoices for your shipments</p>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <table className="w-full">
              <thead className="bg-[#2A2A2A]/50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-right text-xs font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#C5A059]/10">
                {shipments.map((shipment) => (
                  <tr key={shipment.id} className="hover:bg-[#2A2A2A]/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-[#F5F5F0]">{shipment.receiver_name}</td>
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-sm text-[#F5F5F0]/40">Documents will be available once you create shipments</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 p-6">
        <h3 className="text-lg font-semibold text-[#F5F5F0] mb-4">About Your Documents</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-[#C5A059]">AWB Label</h4>
            <p className="text-sm text-[#F5F5F0]/60">
              Air Waybill label contains the barcode and tracking information. Print and attach this to your package.
            </p>
          </div>
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-[#C5A059]">Invoice</h4>
            <p className="text-sm text-[#F5F5F0]/60">
              Commercial invoice with shipment details, pricing breakdown, and tax information.
            </p>
          </div>
        </div>
      </div>
