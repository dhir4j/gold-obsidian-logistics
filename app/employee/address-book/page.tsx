...
export default function EmployeeAddressBookPage() {
  ...
  const handleDelete = async (addressId: number) => {
    ...
    try {
      const response = await fetch(
        ...
        { method: "DELETE", headers: { "X-User-Email": email } }
      );
      if (response.ok) {
        fetchAddresses(email);
      }
    ...
    <div className="group rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 p-6 hover:border-[#C5A059]/40 transition-all duration-300">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-[#F5F5F0]">{address.nickname}</h3>
            <span className="text-xs text-[#F5F5F0]/50 uppercase tracking-wider">{address.address_type}</span>
          </div>
        </div>
        <button
          onClick={() => handleDelete(address.id)}
          className="p-2 rounded-lg text-[#F5F5F0]/60 hover:text-red-400 hover:bg-[#2A2A2A] transition-all opacity-0 group-hover:opacity-100"
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-3xl font-bold text-[#F5F5F0] mb-2">Address Book</h1>
        <p className="text-[#F5F5F0]/60">Save and manage your frequently used addresses</p>
      </div>
        ...
        <div className="flex space-x-1">
          <button
            ...
            className={`px-6 py-3 text-sm font-medium transition-all duration-200 border-b-2 ${
              activeTab === "sender" ? "border-[#C5A059] text-[#C5A059]" : "border-transparent text-[#F5F5F0]/60 hover:text-[#F5F5F0]"
            }`}
          >
          ...
          <button
            ...
            className={`px-6 py-3 text-sm font-medium transition-all duration-200 border-b-2 ${
              activeTab === "receiver" ? "border-[#C5A059] text-[#C5A059]" : "border-transparent text-[#F5F5F0]/60 hover:text-[#F5F5F0]"
            }`}
          >
      ...
      {isLoading ? (
        ...
        <div>
          {(activeTab === "sender" ? senderAddresses : receiverAddresses).length > 0 ? (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {(activeTab === "sender" ? senderAddresses : receiverAddresses).map((address) => (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20">
              ...
              <p className="text-[#F5F5F0]/60 mb-2">No {activeTab} addresses saved</p>
              <p className="text-sm text-[#F5F5F0]/40">
                Add your frequently used {activeTab} addresses for quick booking
              </p>
            </div>
          )}
        </div>
