...
export default function EmployeePaymentsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-3xl font-bold text-[#F5F5F0] mb-2">Billing & Payments</h1>
        <p className="text-[#F5F5F0]/60">View and manage your payment history and invoices</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 p-6">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Total Payments</p>
            <CreditCard className="h-6 w-6 text-[#C5A059]/30" />
          </div>
          <p className="text-3xl font-bold text-[#F5F5F0] font-serif">{payments.length}</p>
        </div>
        <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-green-500/20 p-6">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Approved</p>
        ...
        <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-orange-500/20 p-6">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Pending</p>
      ...
      <div className="rounded-xl bg-gradient-to-br from-[#1a1a1a] to-[#2A2A2A] border border-[#C5A059]/20 overflow-hidden">
        <div className="border-b border-[#C5A059]/20 p-6">
          <h2 className="font-serif text-2xl font-semibold text-[#F5F5F0]">Payment History</h2>
          <p className="text-sm text-[#F5F5F0]/60 mt-1">All your payment transactions and their status</p>
        </div>
          ...
          {isLoading ? (
            ...
            <table className="w-full">
              <thead className="bg-[#2A2A2A]/50">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Shipment ID</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-[#F5F5F0]/60 uppercase tracking-wider">UTR Reference</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Amount</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-[#F5F5F0]/60 uppercase tracking-wider">Date</th>
              ...
              <tbody className="divide-y divide-[#C5A059]/10">
                {payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-[#2A2A2A]/30 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="font-mono text-sm font-medium text-[#C5A059]">{payment.shipment_id_str}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="font-mono text-sm text-[#F5F5F0]/70">{payment.utr}</span>
                    </td>
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-sm text-[#F5F5F0]/40">Your payment transactions will appear here</p>
            </div>
          )}
        </div>
      </div>
